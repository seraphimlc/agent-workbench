...
- 状态：五轮独立审查完成，最后一轮发现项已修订，待用户总评审
- 审查说明：产品范围审查已 APPROVE，运行时/安全专项已 PASS；主规格第五轮仍发现 5 个字段级问题，已逐项修订。按流程上限不再启动第六轮，因此进入用户总评审门。
- 日期：2026-07-13
- 工程：Agent Workbench
- 首发平台：macOS
- 产品阶段：基础框架 MVP
...
根 Turn 在创建事务中先生成 id，并设置 recovery_chain_id=id、queue_kind=normal。恢复 Turn 继承 recovery_chain_id、设置 queue_kind=recovery，并继承原 Turn 的 scenario_contract_mode。非 Scenario Turn 的 contract mode 为 none。

Terminal 状态为 succeeded、failed、canceled、interrupted。Terminal Turn 不得再次变化。

Session runtime_status 是数据库投影，不由 Renderer 或 Runner 自行设置，优先级为 canceling > recovering > running > queued > idle：
...
| Main 60 秒内未接管 | Daemon 取消/中断活动 Turn、保持 queued Turn 但不调度。无 open quarantine 时清理后退出；存在 quarantine 时进入 headless_reaper_only，不接收新任务、不调模型，仅运行 Reaper，全部清除后退出 |
| Daemon 崩溃 | 所有旧 Lease 被新 daemon_epoch fencing；旧 Runner/worker 通过控制通道 EOF 自清理；旧活动 Turn 一律进入 interrupted |
| Daemon 重启且 Main 已连接 | 恢复持久 queued Turn；recovery_review Session 的普通队列保持阻塞，但已持久化 designated recovery Turn 可按 §11.2 越过 |
| 无 active 但有 queued | Daemon 不是 idle，不得自动退出 |
...
| turn.enqueue；turn.cancel；session.create；scenario.start/retryDelivery；artifact.startRevision | 必须 | 必须；响应丢失重试复用原 action token | 必填、持久去重 |
...
4. 只有 Provider 明确结束响应、完整 Tool Call 参数通过 Schema、ModelAttempt 成功事务已提交后，才可逐个提交给 Tool Gateway；interrupted/failed Attempt 的 Tool Call 永远不授权执行。
5. 如果模型调用 skill.invoke，Daemon 创建 SkillRun，用 Skill 的独立输入/输出 Schema 和 Model Profile Role 执行一次结构化模型子调用；Skill 内不再启动 Tool Loop。
6. 将 Tool 或 Skill 结果加入当前 Turn 上下文。
7. 继续模型调用，直到输出最终回答、产生可恢复错误或达到上限。
8. 注册 Agent 明确交付的 final Artifact。
9. 请求 Daemon 事务性完成 Turn；Scenario Turn 先经过 Scenario Supervisor 完成门。
...
- 同一 Session 的普通 Turn 严格 FIFO；用户明确创建的 recovery Turn 优先于该 Session 已存在的普通 queued Turn。
...
3. Scheduler 使用 BEGIN IMMEDIATE claim 事务。queue_block_reason=null 时可选最小 ordinal 普通 Turn；queue_block_reason=recovery_review 时只允许选择与 designated_recovery_turn_id 精确匹配的 queued recovery Turn，普通队列始终不可见。
...
所有 effectful builtin Handler 使用 durable GO handshake：
...
runtime.verify_effect 是 Recovery Supervisor intrinsic：只在 queue_kind=recovery 时自动加入有效能力，不受业务 Scenario.allowed_tools 排除，也不能在 normal/conversation Turn 调用。它只选择并运行原 Tool Manifest 的 postcondition_checker；若 Manifest 没有 Checker，则只能由用户通过 turn.resolveRecovery 写 accepted_unknown，不能伪造 confirmed 状态。
...
| Daemon 崩溃 | 新 boot epoch fencing 所有旧 Lease；worker 经控制通道 EOF 自清理；活动 Turn 标记 interrupted |
...
## 24. 推荐 Monorepo 结构
    ...
    agent-workbench/
      ...
      packages/
        ...
        extension-sdk/           声明式清单、Schema、digest/cache 加载器；无第三方可执行 ABI
        ui/                      共享 UI 组件
...
5. 声明式 Extension Loader 与数据报告 Reference Scenario。
6. Foundation Core 完整 E2E、崩溃注入和安全检查。
7. 电商 Listing Validation Pack 和真实豆包 Smoke Test。
8. 基础框架稳定后，再为 fbm-pipeline 编写独立集成规格。
...
|---|---|---|
| queued | running | Session 未阻塞且普通队首合法，或该 Turn 是 blocked Session 的 designated recovery；slot CAS 成功 |
| queued | canceled | 当前仍为 queued |
| running | succeeded | 未提交 cancel_requested，Scenario completion gate 通过 |
...
|---|---|---|
| queued | running | effectful 调用已提交 Audit intent，worker READY，事务写 go_sent；effect_state=unknown |
...
| Daemon 崩溃 | 新 daemon_epoch fencing；旧活动 Turn interrupted，不续跑旧 Runner |
